...
BEGIN
	...
	-- Automation: Create Table
	create table if not exists dm.dm_total_transaction_and_user_hour as
	select 
	...
	-- Automation: Truncate --> SCD Type 1
    truncate table dm.dm_total_transaction_and_user_hour;

	-- Automation: Inserting New Data
	insert into dm.dm_total_transaction_and_user_hour
	select 
		extract(hour from transaction_time) as transaction_hour,
		count(sale_id) as total_transaction,
		count(distinct user_id) as total_user,
	...
	-- Automation: Create Table
	create table if not exists dm.dm_total_qty_product as
	select
		product_id,
	...
	-- Automation: Truncate --> SCD Type 1
    truncate table dm.dm_total_qty_product;

	-- Automation: Inserting New Data
    insert into dm.dm_total_qty_product
	select
		product_id,
		sum(quantity) as total_quantitiy,
		current_timestamp as last_update
	...
	-- Automation: Create Table
	create table if not exists dm.dm_total_qty_store as
	select
		store_id,
		store_name,
	...
	-- Automation: Truncate --> SCD Type 1
    truncate table dm.dm_total_qty_store;
 
	-- Automation: Inserting New Data
    insert into dm.dm_total_qty_store
	select
		store_id,
		store_name,
		sum(quantity) as total_quantity,
    ...
    -- Step 14: Additional DM (rename to your datamart) Total quantity by category
	-- Automation: Create Table
	create table if not exists dm.dm_total_qty_category as
	select
		product_category,
		sum(quantity) as total_quantity,
		current_timestamp as last_update
	...
	-- Automation: Truncate --> SCD Type 1
    truncate table dm.dm_total_qty_category;

	-- Automation: Inserting New Data
    insert into dm.dm_total_qty_category
	select
		product_category,
		sum(quantity) as total_quantity,
		current_timestamp as last_update
